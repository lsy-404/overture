...
const recipe: Recipe = {
  schema: 2, id: "demo", name: "Demo", summary: "Demo", version: "1.0.0", tag: "v1.0.0", buildTime: "2026-01-01T00:00:00Z",
  ...
  worker: {
    ...
    containers: [
      { className: "Sandbox", mode: "ask" },
      { className: "Required", mode: "always" },
    ],
  },
};
...
const freshDefaults = wizard.containerActions.Sandbox === "off" && wizard.containerActions.Required === "on";
const freshTarget = wizard.buildTarget();
const freshDeclaration = freshTarget.declareContainers.join(",") === "Required" && freshTarget.containerActions?.Sandbox === "off";

wizard.applyLive({ exists: true, vars: {}, crons: [], customDomains: [], containerClasses: ["Sandbox", "Required"] });
const overwriteDefaults = wizard.containerActions.Sandbox === "unchanged" && wizard.containerActions.Required === "on";
const unchangedPreservesLive = wizard.declareContainers.join(",") === "Sandbox,Required";
...
const uiUsesThreeStates = /value="unchanged"/.test(targetSource) && /value="on"/.test(targetSource) && /value="off"/.test(targetSource);
